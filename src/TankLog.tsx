        ...
        {batch.packaging && (
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11.5, color: "#5C6B63", marginTop: 6 }}>
            {(() => {
              ...
              return parts.join(" · ") || "No containers logged";
            })()}
          </div>
        )}
      </div>
...
function DiscardRemainingModal({ batch, onClose, onConfirm }) {
  ...
  return (
    <Modal title={`Discard remaining — ${batch.name}`} onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ color: "#8A9591", fontSize: 13 }}>
          There's <span style={{ color: "#EDE7D9", fontFamily: "'JetBrains Mono', monospace" }}>{remaining} L</span> still sitting in
          the tank for this batch. This logs it as loss (trub, dead space, spillage, etc.) and closes the batch out —
          it won't show as outstanding anymore.
        </div>
        <button
          onClick={submit}
          style={{
